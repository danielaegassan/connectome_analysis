...
def bidirectional_edges(sparse_matrix: Union[sp.csr_matrix, sp.csc_matrix]) -> Union[sp.csr_matrix, sp.csc_matrix]:
    """
    Returns the bidirectional edge matrix of the input.

    :param sparse_matrix: Sparse input matrix.
    :type: Union[sp.csr_matrix, sp.csc_matrix]
    
    :return bedge_matrix: Sparse bedge matrix.
    :rtype: Union[sp.csr_matrix, sp.csc_matrix]
    """
    return sparse_matrix.multiply(sparse_matrix.T)
...
def non_overlapping_subsampled_matrices(sparse_matrix: sp.coo_matrix, ns: List[int], generator:np.random.Generator) -> List[sp.coo_matrix]:
    ...
    """
    selection = generator.choice(len(sparse_matrix.col), replace=False, size=sum(ns))
    return [subset_matrix(sparse_matrix, selection[ns[i]:ns[i+1]]) for i in range(ns-1)]
...
def half_matrix(sparse_matrix: sp.coo_matrix, generator: np.random.Generator) -> List[sp.coo_matrix]:
    ...
    return subset_matrix(sparse_matrix, permutation[:int(len(permutation)/2)]), subset_matrix(sparse_matrix, permutation[int(len(permutation)/2):])


def adjust_bidirectional_connections(sparse_matrix: sp.csc_matrix, bedges_to_add: int, generator:np.random.Generator) -> sp.csc_matrix:
    """
    ...
    :rtype: sp.csc_matrix
    """
    bedge = bidirectional_edges(sparse_matrix)
    dedge = sparse_matrix - bedge
    dedge_coo = dedge.tocoo(copy=False) # Easier to subsample
    dedges_to_bedge, dedges_to_remove = non_overlapping_subsampled_matrices(
        dedge_coo, [bedges_to_add, bedges_to_add], generator
    )# Coo matrices are auto casted before sum 
    return sparse_matrix + dedges_to_bedge.T - dedges_to_remove


def add_bidirectional_connections(sparse_matrix: sp.csc_matrix, bedges_to_add: int, generator:np.random.Generator) -> sp.csc_matrix:
    """
    Turn a fixed amount of directional connections into bidirectional connections.
